...
export interface TypeHzCustomTableColumns {
  ...
  title: string
  render?(
    row: TypeHzCustomTableRow,
    col: TypeHzCustomTableColumns
  ): ReactNode | string
  [index: string]: any
}
...
const HzCustomTable: FC<HzCustomTableProps> = props => {
  ...
  const { custom_table, thead, tbody, table_row, table_col, operate_btns } =
    styles
  return (
    <div className={custom_table}>
      <div className={thead}>
        <div className={table_row}>
          {columns.map((col, idx) => {
            return (
              <div key={idx} style={{ width: col.width }}>
                {col.title}
              </div>
            )
          })}
        ...
        {dataSource.map((row, rowIdx) => {
          return (
            <div
              key={rowIdx}
              className={table_row}
              style={{ height: rowHeight && rowHeight + 'px' }}
            >
